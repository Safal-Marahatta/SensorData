...
export default function AlertsAndPlansDisplay(): JSX.Element {
  const [mockAlerts, setAlerts] = useState<AlertRecipient[]>([]);
  ...
  useEffect(() => {
    const fetchAlerts = async () => {
      try {
        ...
        const data: AlertRecipient[] = await response.json();
        setAlerts(data);
      } catch (err) {
        console.error("Error fetching alerts:", err);
      } finally {
    ...
    <>
      ...
      <div className="container mx-auto bg-gradient-to-br from-gray-700 to-gray-700 text-gray-100 min-h-screen">
            ...
            <TableBody>
              {planFeatures.map((feature, index) => (
                <TableRow key={index} className="hover:bg-gray-700/50 transition-colors duration-200">
                  ...
                  <TableCell className="text-center">
                    <div className="flex justify-center">
                      {feature.full && <Checkbox checked={feature.full} />}
                    </div>
                  </TableCell>
                  <TableCell className="text-center">
                    <div className="flex justify-center">
                      {feature.limited && <Checkbox checked={!feature.limited} />}
                    </div>
                  </TableCell>
                </TableRow>
              ))}
        ...
        <p className="text-white pt-8 text-center">
          For any technical support, please contact: info@ies.com.np or +977 9851198185
        </p>
      </div>
    </>
  );
